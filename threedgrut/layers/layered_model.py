...
class LayeredGaussians(nn.Module):
    ...
    def get_model_parameters(self) -> dict:
        ...
        # NOTE: ``_track_albedo_table`` / ``_track_log_scale_table`` would
    ...
    def init_from_checkpoint(self, checkpoint: dict, setup_optimizer: bool = True):
        ...
        if nodes_dict is not None:
            ...
            # V3-L8/L9: restore per-track albedo / log-scale tables + Adam
            # state. Looked up under either NRE-wrapped or unwrapped key;
            # absent entries leave tables untouched (so an OFF-config load on
            # an ON-trained ckpt produces a clear warning + safe fallback
            # rather than crashing).
            track_optim_state = None
            if (
                "model" in checkpoint
                and isinstance(checkpoint["model"], dict)
                and "track_optim_state" in checkpoint["model"]
            ):
                track_optim_state = checkpoint["model"]["track_optim_state"]
            elif "track_optim_state" in checkpoint:
                track_optim_state = checkpoint["track_optim_state"]
            ...
            # NOTE: this runs AFTER track_optim_state restore so any
            # ``_track_albedo_table`` / ``_track_log_scale_table`` entries in
            # layered_track_state will overwrite — both blocks write the same
            # values when both are present (V3-L8/L9 + Stage A on same ckpt).
            layered_track_state = None
            if (
                "model" in checkpoint
                and isinstance(checkpoint["model"], dict)
                and "layered_track_state" in checkpoint["model"]
            ):
                layered_track_state = checkpoint["model"]["layered_track_state"]
