...
def test_pinhole_straddling_plane_aabb_from_visible_corners_only():
    ...
    """
    pose = torch.eye(4); pose[:3, 3] = torch.tensor([0., 0., 1.0])
    sizes = torch.tensor([[1., 1., 2.4]])
    mask = project_cuboids_to_mask(
        pose.unsqueeze(0), sizes, _K(), _identity_pose(),
        H=512, W=512, device="cpu",
    )
...
def test_pinhole_mixed_tracks_behind_one_dropped_front_one_kept():
    ...
    pose_behind = torch.eye(4); pose_behind[:3, 3] = torch.tensor([0., 0., -5.])
    pose_front = torch.eye(4); pose_front[:3, 3] = torch.tensor([0., 0., 5.])
    sizes = torch.tensor([[1., 1., 1.], [1., 1., 1.]])
    mask_both = project_cuboids_to_mask(
        torch.stack([pose_behind, pose_front]), sizes,
        _K(), _identity_pose(), H=512, W=512, device="cpu",
    )
    mask_front_only = project_cuboids_to_mask(
        pose_front.unsqueeze(0), sizes[1:],
        _K(), _identity_pose(), H=512, W=512, device="cpu",
    )
...
def test_resolve_intrinsics_pinhole_builds_K_from_numpy():
    ...
    K, ftheta = resolve_batch_cuboid_intrinsics(_pinhole_batch(
        focal=np.asarray([500.0, 501.0]), pp=np.asarray([256.0, 257.0]),
    ))
...
def test_resolve_intrinsics_pinhole_accepts_torch_and_list():
    from threedgrut.layers.dynamic_mask import resolve_batch_cuboid_intrinsics
    K_t, _ = resolve_batch_cuboid_intrinsics(_pinhole_batch(
        focal=torch.tensor([400.0, 400.0]), pp=torch.tensor([100.0, 200.0]),
    ))
    K_l, _ = resolve_batch_cuboid_intrinsics(_pinhole_batch(
        focal=[400.0, 400.0], pp=[100.0, 200.0],
    ))
...
def test_resolve_intrinsics_ftheta_takes_precedence():
    ...
    K, ftheta = resolve_batch_cuboid_intrinsics(_pinhole_batch(
        focal=[500.0, 500.0], pp=[256.0, 256.0], ftheta=fp,
    ))
    assert K is None
    assert ftheta is fp
