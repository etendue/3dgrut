...
def test_road_slab_mask_geometry():
    from threedgrut.model.road_reg import bg_in_road_slab_mask, build_road_bev_height
...
def test_bg_road_slab_exclude_clamps_in_slab_only():
    ...
    gx, gy = torch.meshgrid(xs, xs, indexing="ij")
    road = _layer(torch.stack([gx.flatten(), gy.flatten(), torch.zeros(36)], -1), torch.zeros(36, 1))
