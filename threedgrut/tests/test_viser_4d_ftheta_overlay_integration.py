...
def test_compositor_draws_cuboid_on_gray_backdrop():
    ...
    """
    cmp = Viser4DOverlayCompositor(_ftheta_dict(), height=1080, width=1920, subdivide_n=5)
    backdrop = np.full((1080, 1920, 3), 128, dtype=np.uint8)
    ...
    assert green_mask.sum() > 50, (
        f"expected at least 50 green pixels from cuboid wireframe; " f"got {int(green_mask.sum())}"
    )
...
def test_compositor_layer_ordering_preserved():
    ...
    cmp = Viser4DOverlayCompositor(_ftheta_dict(), height=1080, width=1920, subdivide_n=3)
    backdrop = np.full((1080, 1920, 3), 128, dtype=np.uint8)

    # Same cuboid drawn twice: first red, then blue. Final pixels should be blue.
    pose = np.eye(4, dtype=np.float32)
    pose[2, 3] = -10.0
    edges = cuboid_world_edges(pose, np.array([2.0, 2.0, 2.0], dtype=np.float32))
    polylines = [edges[i] for i in range(12)]

    layers = [
        PolylineLayerSpec(name="L_red", polylines_world=polylines, color=(255, 0, 0, 255), width=3),
        PolylineLayerSpec(name="L_blue", polylines_world=polylines, color=(0, 0, 255, 255), width=3),
    ]
    ...
    assert blue_mask.sum() > red_mask.sum(), (
        f"blue (top) should dominate red (bottom); " f"blue_px={int(blue_mask.sum())} red_px={int(red_mask.sum())}"
    )
...
def test_compositor_draws_label_at_projected_anchor():
    ...
    cmp = Viser4DOverlayCompositor(_ftheta_dict(), height=1080, width=1920, world_to_camera_flip=np.eye(4))
    backdrop = np.full((1080, 1920, 3), 128, dtype=np.uint8)
