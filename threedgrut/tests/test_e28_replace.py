...
from threedgrut.layers.e28_replace import VEHICLE_CLASSES, assign_assets_to_tracks
...
def test_non_vehicle_track_particles_unchanged(monkeypatch):
    ...
    recon = {"0": ("automobile", (4, 2, 1.5)), "1": ("car", (4, 2, 1.5)), "2": ("VRU_pedestrians", (0.6, 0.6, 1.7))}
...
def test_select_present_always_kept_insert_filtered():
    ...
    recon, name_to_id = select_vehicle_tracks_to_place(catalog, min_active_frames=20, max_dist_m=40.0)
...
from threedgrut.layers.e28_replace import (
    _sorted_dims_ratio,
    extract_recon_node_tensors,
    keep_only_track_slots,
    place_tracks_in_dyn_node,
...
def test_split_routes_oversized_to_recon():
    ...
    sel = {"car": ("automobile", (4.5, 1.8, 1.5)), "bus": ("bus", (12.5, 3.1, 3.5))}
...
def test_place_tracks_insert_keeps_existing():
    ...
    nt = {
        5: {
            ...
            "rotation": torch.ones(2, 4),
            "scale": torch.ones(2, 3),
            "density": torch.ones(2, 1),
            "features_albedo": torch.ones(2, 3),
            "features_specular": torch.ones(2, 2),
        }
    }
...
def test_extract_recon_node_tensors_adjusts_spec_dim():
    ...
    rdyn = {
        ...
        "scale": rp(3, 9),
        "density": rp(1, 9),
        "features_albedo": rp(3, 9),
        "features_specular": rp(4, 9),
        "track_ids": rtids,
    }
