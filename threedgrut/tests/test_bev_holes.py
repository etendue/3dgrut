...
def test_all_transparent_gives_full_A_rate_and_zero_coverage():
    ...
    ego = _ego_line()
    s = compute_bev_hole_stats(xy, op, ego, cell_size=0.5, corridor_half_width=1.5, opacity_floors=(0.1,))
...
def test_all_opaque_gives_full_coverage_zero_holes():
    ...
    ego = _ego_line()
    s = compute_bev_hole_stats(xy, op, ego, cell_size=0.5, corridor_half_width=1.5, opacity_floors=(0.3,))
...
def test_far_particle_outside_corridor_is_ignored():
    xy, op = _tile_rect(-3, 23, -2.5, 2.5, step=0.4, opacity=0.9)
    ego = _ego_line()
    base = compute_bev_hole_stats(xy, op, ego, cell_size=0.5, corridor_half_width=1.5, opacity_floors=(0.3,))
    ...
    s = compute_bev_hole_stats(xy2, op2, ego, cell_size=0.5, corridor_half_width=1.5, opacity_floors=(0.3,))
...
def test_grid_bounded_to_corridor_not_particle_sprawl():
    ...
    ego = _ego_line(0, 20, 1.0)
    s = compute_bev_hole_stats(sprawl_xy, sprawl_op, ego, cell_size=0.5, corridor_half_width=12.0)
...
def test_length_mismatch_raises():
    with pytest.raises(ValueError):
        compute_bev_hole_stats(np.zeros((10, 2)), np.zeros(9), _ego_line())
