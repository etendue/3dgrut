...
from threedgrut.model.nta_iou import (
    compute_frame_nta_iou,
    project_track_to_2d_box,
)
...
def test_project_track_front_center_box():
    ...
    box = project_track_to_2d_box(
        pose, size, K=_pinhole_K(), ftheta_params=None,
...
def _front_vehicle_track():
    ...
    pose[2, 3] = 10.0
    return {
        "id": 1, "class": "automobile",
        "pose": pose, "size": torch.tensor([2.0, 2.0, 2.0]),
    }
...
def test_nta_iou_perfect_match():
    ...
    gt = project_track_to_2d_box(
        track["pose"], track["size"], _pinhole_K(), None, torch.eye(4), 480, 640,
    )
    det = _FakeDetector([list(gt)])  # detection box == GT box
    out = compute_frame_nta_iou(
        torch.zeros(480, 640, 3), [track], det,
...
def test_nta_iou_no_detection_scores_zero():
    track = _front_vehicle_track()
    out = compute_frame_nta_iou(
        torch.zeros(480, 640, 3), [track], _FakeDetector([]),
        K=_pinhole_K(), ftheta_params=None, T_w2c=torch.eye(4), H=480, W=640,
    )
...
def test_nta_iou_no_gt_vehicle_returns_none():
    ped = {
        "id": 9, "class": "pedestrian",
        "pose": torch.eye(4), "size": torch.tensor([1.0, 1.0, 2.0]),
    }
    out = compute_frame_nta_iou(
        torch.zeros(480, 640, 3), [ped], _FakeDetector([[0, 0, 10, 10]]),
        K=_pinhole_K(), ftheta_params=None, T_w2c=torch.eye(4), H=480, W=640,
    )
    assert out is None
...
def test_nta_iou_novel_pose_shifts_gt_box():
    ...
    box_orig = project_track_to_2d_box(
        track["pose"], track["size"], _pinhole_K(), None,
        torch.linalg.inv(c2w), 480, 640,
    )
    ...
    out = compute_frame_nta_iou(
        torch.zeros(480, 640, 3), [track], det,
        K=_pinhole_K(), ftheta_params=None, T_w2c=T_w2c_novel, H=480, W=640,
    )
    assert out["mean_nta_iou"] > 0.99
