...
def _guards():
    return {
        "comparison": {
            "background_on_road_alpha_reduction_fraction_min": 0.5,
...
def test_each_bad_metric_fails_its_guard():
    cases = [
        ("ownership", "bg_on_road_alpha_mean", 0.8),
...
def test_same_budget_quality_baseline_overrides_30k_quality_reference_only():
    ...
    bg = next(c for c in report["checks"] if c["name"] == "background_on_road_alpha")
    assert bg["limit"] == pytest.approx(0.495)
