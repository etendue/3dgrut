...
def test_psnr_in_mask_only_inside_mask_counted():
    ...
    mask = torch.zeros(H, W); mask[:32] = 1.0
    out = compute_psnr_in_mask(rgb_pred, rgb_gt, mask)
...
def test_psnr_in_mask_too_few_pixels_returns_none():
    H, W = 64, 64
    rgb = torch.rand(H, W, 3)
    mask = torch.zeros(H, W); mask[0, 0] = 1.0  # 1 pixel only
...
def test_compute_class_psnr_empty_active_tracks_returns_no_psnr():
    ...
    out = compute_class_psnr(
        rgb_pred, rgb_gt, valid_mask=None,
        active_tracks=[],
        T_world2cam=_identity_pose(), H=64, W=64, K=_pinhole_K(),
    )
...
def test_compute_class_psnr_one_active_track_known_psnr():
    ...
    out = compute_class_psnr(
        rgb_pred, rgb_gt, valid_mask=None,
        active_tracks=[{"id": "t0", "class": "automobile",
                        "pose": pose, "size": size}],
        T_world2cam=_identity_pose(), H=H, W=W, K=_pinhole_K(),
    )
...
def test_compute_class_psnr_by_class_aggregation():
    ...
    for i, (cls, x_shift) in enumerate([
        ("automobile", -0.5), ("automobile", 0.5), ("heavy_truck", 1.5),
    ]):
        pose = torch.eye(4)
        pose[:3, 3] = torch.tensor([x_shift, 0.0, 5.0])
        tracks.append({"id": f"t{i}", "class": cls,
                       "pose": pose,
                       "size": torch.tensor([1.0, 1.0, 1.0])})
...
def test_compute_class_psnr_track_outside_image_yields_zero_pixels():
    ...
    rgb_gt = torch.zeros(1, H, W, 3)
    pose = torch.eye(4); pose[:3, 3] = torch.tensor([0.0, 0.0, -5.0])  # behind cam
    out = compute_class_psnr(
        rgb_pred, rgb_gt, valid_mask=None,
        active_tracks=[{"id": "t0", "class": "automobile",
                        "pose": pose, "size": torch.tensor([1.0, 1.0, 1.0])}],
        T_world2cam=_identity_pose(), H=H, W=W,
        ftheta_params={"angle_to_pixeldist_poly": [0.0, 200.0],
...
def test_compute_class_psnr_valid_mask_excludes_invalid_pixels():
    ...
    pose = torch.eye(4); pose[:3, 3] = torch.tensor([0.0, 0.0, 5.0])
    out = compute_class_psnr(
        rgb_pred, rgb_gt, valid_mask=valid,
        active_tracks=[{"id": "t0", "class": "automobile",
                        "pose": pose, "size": torch.tensor([1.0, 1.0, 1.0])}],
        T_world2cam=_identity_pose(), H=H, W=W, K=_pinhole_K(),
    )
...
def test_compute_class_psnr_with_ftheta_intrinsics():
    ...
    out = compute_class_psnr(
        ...
        active_tracks=[{"id": "t0", "class": "automobile",
                        "pose": pose, "size": torch.tensor([1.0, 1.0, 1.0])}],
        T_world2cam=_identity_pose(), H=H, W=W,
        ftheta_params={"angle_to_pixeldist_poly": [0.0, 300.0],
