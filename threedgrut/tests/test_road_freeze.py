...
def test_apply_layer_lr_overrides_absolute_and_drops_scheduler() -> None:
    LayeredGaussians = pytest.importorskip("threedgrut.layers.layered_model").LayeredGaussians
    layer = _mock_layer()
    spec = LayerSpec(
        name="road",
        layer_id=2,
        max_n_particles=200000,
        positions_lr=1e-6,
        density_lr=1e-4,
        rotation_lr=1e-4,
        scale_lr=1e-4,
...
def test_post_backward_zeros_road_rotation_grad() -> None:
    LayeredMCMCStrategy = pytest.importorskip("threedgrut.strategy.layered_mcmc").LayeredMCMCStrategy
    ...
    specs = [
        ...
        LayerSpec(name="road", layer_id=1, max_n_particles=200000, freeze_rotation_grad=True),
    ]
    mock_self = _mock_strategy_self(specs, layers)
    out = LayeredMCMCStrategy._post_backward(mock_self, step=1, scene_extent=1.0, train_dataset=None)
...
def test_post_backward_noop_when_freeze_false() -> None:
    ...
    mock_self = _mock_strategy_self(specs, layers)
    LayeredMCMCStrategy._post_backward(mock_self, step=1, scene_extent=1.0, train_dataset=None)
