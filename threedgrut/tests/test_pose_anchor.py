...
def test_boundary_zero_when_learned_equals_gt():
    ...
    t = torch.randn(F, 3)
    q = _yaw_quat(F)
    model = _AnchorModelStub({"t0": {"trans": t, "quat": q, "active": torch.ones(F, dtype=torch.bool)}})
...
def test_boundary_endpoints_follow_active_mask():
    ...
    m_int = _AnchorModelStub(
        {"t0": {"trans": learned_interior, "quat": _identity_quat(F), "active": active, "gt_trans": gt_t}}
    )
    ...
    m_edge = _AnchorModelStub(
        {"t0": {"trans": learned_edge, "quat": _identity_quat(F), "active": active, "gt_trans": gt_t}}
    )
...
def test_prior_ignores_inactive_frames():
    ...
    model = _AnchorModelStub(
        {"t0": {"trans": learned_t, "quat": _identity_quat(F), "active": active, "gt_trans": gt_t}}
    )
...
def test_trainer_prior_freeze_window_returns_zero():
    ...
    conf = SimpleNamespace(
        learnable_pose=SimpleNamespace(
            lambda_pose_prior_trans=1.0,
            lambda_pose_prior_rot=1.0,
        )
    )
