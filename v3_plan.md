...
| **V3-VIZ.6** | VIZ (patch) | **dynamic track 单帧漏检补洞（去回放闪烁）** — viser_gui_4d 回放发现 track **259**（automobile，左前方轿车 4.48×1.95×1.66m，cam 系 z≈14.6m / x≈-12.9m）在**第 406 帧 `frame_info=False` 单帧消失**（405/407 均 active），整段共 **4 处单帧洞** idx `[109,139,391,406]`，系上游 tracker 偶发漏检（**非图片缺失** —— viewer 实时渲高斯不读原图；时间戳单调无别名 dt≈33ms，故非全局跳帧）。**根因坐实**（ThinkPad ckpt 实测，2026-06-03）：高斯车（`dynamic_rigids`）按 `_track_active_<tid>[idx]` 门控，inactive → density 压 −50 → sigmoid≈0 → 整车不渲染（[layered_model.py:1014](threedgrut/layers/layered_model.py:1014)）；cuboid 线框走同源 `frame_info`（[viz4d_metadata.py:191](threedgrut_playground/utils/viz4d_metadata.py:191)）→ 二者同帧一起消失。**Fix**：新增 `fill_track_pose_holes(tracks, max_gap=K)` helper —— 检测 `frame_info` 短空洞（两侧 active、长度 ≤K）→ translation 线性插值 + quaternion slerp 补 `poses`、置 `frame_info=True`，在 `populate_tracks`（覆盖 render.py + viser 两条 reload 路径，见 V3-E4.1）统一调用；同步引擎 `_track_active_/_track_pose_` buffer。默认 viz/playground 开（K=2），训练侧 `opt-in` flag（默认关，避免改监督口径 / 保持 byte-identical）。**验收**：回放 406 帧车不再闪 + Mac 单测 pin 住插值（单帧/双帧洞补、端点洞不补、>K 洞不补）+ 训练 flag 关时 byte-identical。 | V3-VIZ / 数据质量 | 0.5 | ⬜ | — |
...
**关联未来 task**：
- **V3-P2** — 修 dyn→bg/road 错分（cuboid-exclusion mask 加严 / 训练后 post-process 粒子迁移）。
- **V3-VIZ.6**（patch，⬜ 待办，见 §1.2 任务表）— dynamic track 单帧漏检补洞：回放发现 track 259 第 406 帧 `frame_info=False` 单帧消失（上游 tracker 偶发漏检，非图片问题），整段 4 处单帧洞 `[109,139,391,406]`；fix 走 `populate_tracks` 加 `fill_track_pose_holes` 短空洞插值。
