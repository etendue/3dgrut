...
def save_folder2video(save_dir, remove_key=0):
    ...
    imageio.v2.mimwrite(
        video_file.replace("video.mp4", "video_10fps.mp4"), ims[::3], fps=10, macro_block_size=1
    )  # type: ignore
    imageio.v2.mimwrite(
        video_file.replace("video.mp4", "video_15fps.mp4"), ims[::2], fps=15, macro_block_size=1
    )  # type: ignore
...
def diffuse_step(
    model, condition, sigma_B_T, batch_size: int, c: int, h: int, w: int, dtype: torch.dtype, device: torch.device
):
    compression = 8
    x = torch.randn(batch_size, 16, 1, h // compression, w // compression, dtype=dtype, device=device)

    with torch.autocast(device_type="cuda", dtype=dtype, enabled=True):
        output = model.denoise(xt_B_C_T_H_W=x, sigma=sigma_B_T, condition=condition).x0
...
def model_inference(model, batch_size: int, h: int, w: int, dtype: torch.dtype, device: torch.device, x=None):
    if x is None:
...
def warmup_model(
    model: torch.nn.Module, batch_size: int, h: int, w: int, dtype: torch.dtype, device: torch.device, n: int = 10
) -> None:
...
def load_and_compile_model(
    ...
    model = Pix2Pix_Turbo(
        pretrained_path=model_path, timestep=timestep, vae_skip_connection=vae_skip_connection, batch_size=batch_size
