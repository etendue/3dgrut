...
| `threedgrut/datasets/aux_readers.py` | T3.1.b ✅ (5b49f4b, SsegAuxReader + LidarSsegAuxReader 直读 itar) + **P0.2 ✅ (4a9f2e6, EgomaskAuxReader 通用相机组发现 + resolve_ego_valid_mask 三分支 valid 图纯函数，12 Mac 单测；cecb6b0 T2 升级 resolve_ego_valid_mask_with_source 带 source 标签 "sdk"/"itar"/"none"，原函数收缩 thin wrapper 保持 API 不变)** |
...
| `threedgrut/tests/test_egomask_aux_reader.py` | **P0.2 ✅ (4a9f2e6 + cecb6b0)** — EgomaskAuxReader + resolve_ego_valid_mask 12 单测（并集/PNG bytes/三分支/dilation_iters=0 guard）+ 4 wire 测（fallback 像素计数 / PAI 逐字节等价 / source enum / wrapper 签名回归） |
...
| `threedgrut_playground/viser_gui_4d.py` | T8.3-T8.6 ✅ (Stage 8 Mac, ~640 行 Viser4DViewer + 5 folder GUI + play_tick + timeline 互锁 + ego/LiDAR/axes/frustum + tracks polylines + per-frame cuboid wireframe + --dataset_path lazy fallback) · T8.10 `--no_gaussian_render` Ampere 兼容 ✅ · T8.11 dyn LiDAR per-frame transform ✅ · T8.12 Reset View snap initial_c2w ✅ · T8.12-FIX `--initial_fov_deg`/`--camera_type`/`--camera_fov_deg` CLI flags + `_on_connect` 显式 fov ✅ · T8.13 FTheta intrinsics 透传 + W×H 锁定 + slider 隐藏 + markdown 提示 ✅ · **mixed-camera fix (2026-07-14)** `CameraRenderState` 单一真源；initial camera/dropdown/engine/client/overlay/frustum 原子同步；FTheta↔OpenCVPinhole 清理 stale state；两模型 calibrated image-space overlay；Follow Camera translation lerp + quaternion SLERP + gap diagnostics；C4 inceptio switch matrix 通过（radial native/viewer comparator 已实现，exact UI-free viewer dump 待补） · **T8.14** `_build_static_gui` Render Controls 内嵌 "Gaussian Layers" 子 folder + isinstance/engine 守卫 + per-layer checkbox + closure 默认参数绑定 + callback 整体替换 `enabled_layer_names` ✅ · **38a5af1** `--renderer {3dgrt,3dgut}` CLI flag + GUI Renderer 下拉框 + `engine.set_renderer()` 热切换；A800 无 RT 核可用 ✅ · **difix** `--difix_server` CLI + `DifixClient` 钩子 + `_maybe_difix` 每帧路由 + DiFix toggle/RTT GUI（单帧后处理） ✅ · **E2.6 (5cf26e7)** `--harmonizer_temporal_server`/`--harmonizer_temporal_K` CLI（与 `--difix_server` 互斥）+ `HarmonizerTemporalClient` 构造 + `_postproc_reset_flag`（`_on_time_change` 中 `source!="play"` 即 set，seek/scrub/loop-wrap 自动 reset 历史）+ `_postproc_last_wh` 分辨率锁 + `_maybe_difix` 改分发（temporal 优先走 1+K 带 reset，否则原 DifixClient 路径不变，加 None-guard 防 AttributeError）；新增"Harmonizer (temporal, de-flicker)" checkbox ✅ |
...
| `threedgrut/layers/dynamic_mask.py` (v5-A5) | pinhole 分支补 behind-camera z>0 corner 过滤（与 FTheta 分支共用 `_aabb_from_visible_corners`，FTheta 数值路径逐字节不动）；新增 `resolve_batch_cuboid_intrinsics(gpu_batch)→(K, ftheta)` 双模型 dispatch（FTheta 优先；pinhole 忽略畸变系数——AABB 粒度可接受） | v5-A5 ✅ (297a0bc) |
| `threedgrut/trainer.py` (v5-A5/A1) | `_maybe_fill_cuboid_mask` 补 pinhole K 路径（首次填充打 `[A5]` INFO）；backward 前 **NaN 哨兵**（非有限 total_loss 带 camera/frame/ts/nan_px 坐标 raise）；forward 后**非有限 pred 丢帧 guard**（0·NaN=NaN 梯度穿透唯一气密解，`NONFINITE_PRED_POLICY=sanitize` 为实验开关）；`compute_metrics` eval 侧 GT 替换 | v5-A5 ✅ / v5-A1 ✅ |
| `threedgrut/render.py` (v5-A5/A1) | class_psnr eval gate 放宽双模型 dispatch（pinhole clip 缺 `mean_class_psnr` 字段的根因）；NTA-IoU 同步传 K；eval loop 非有限 pred GT 替换 + metrics.json `nonfinite_pred_px` 透明计数 | v5-A5 ✅ / v5-A1 ✅ |
| `threedgrut/datasets/tracks_loader.py` (v5-A2) | 新纯函数 `interp_pose_to_ts`（平移 lerp + 四元数 slerp 短弧，区间外容差内钳位端点）+ `load_tracks_from_ncore_cuboids(pose_time_mode=)`（默认 `nearest` 逐字节等价；`interp` 同时改为在 obs 自身时刻做 rig→world 变换）+ `CUBOID_TS_MODES` / `build_cuboid_frame_timeline_us`（ref 单相机 vs 全相机 END ts 并集） | v5-A2 ✅ (6983018) |
| `configs/dataset/ncore.yaml` (v5-A2) | + `cuboid_ts_mode: ref_nearest`（默认旧行为；`per_camera_interp` 开启 A2） | v5-A2 ✅ |
| `threedgrut/datasets/datasetNcore.py` (v5-A1) | rays 预计算后 `repair_nonfinite_rays` 极点防护（rational 畸变分母极点落图像内 → NaN ray 修复 + 像素写 ego valid mask=False 永不监督） | v5-A1 ✅ (52224b3) |
| `threedgrut/datasets/utils.py` + `threedgrut/utils/misc.py` (v5-A1) | + `repair_nonfinite_rays`（同行最近有限邻居回退）；+ `replace_nonfinite_pixels`（eval 专用 GT 替换，训练侧禁用——梯度泄漏 hazard 有测试钉死） | v5-A1 ✅ |
| `threedgrut/strategy/mcmc.py` (v5-A1) | `relocate_gaussians` 全死层守卫（alive 空 → 跳过+告警，防空 multinomial CUDA 崩溃）；`sample_new_gaussians` 输入 donor density clamp ≤1-eps + 输出 `_sanitize_relocation`（Eq.9 kernel 非有限/非正行回退 donor 纯复制） | v5-A1 ✅ (5f62cb0/0b960eb) |
...
| **P0.2/P0.3 egomask fallback 字节等价**：`resolve_ego_valid_mask` 三分支（SDK 非零→现路径 / SDK 空+itar 命中→itar 并集→dilate→取反 / 都无→全 True）——**无 egomask itar 的 clip（PAI 线）SDK 非零则走现路径、SDK 无则 branch 3 全 True，与 datasetNcore 现路径逐字节一致**；`dilation_iters=0` 恒等 guard（scipy iterations<1 会膨胀到收敛，用 iters>=1 才调 scipy；datasetNcore 默认 30 → 现路径字节等价）。b6a9 clip 目录仅数据文件替换（新 itar 91KB，旧 161KB 备份至 aux_backup/），下游代码零改动 | P0.2 ✅ (4a9f2e6 + cecb6b0 datasetNcore 接线) + P0.3 ✅ (40277d2) | `test_egomask_aux_reader.py` 16 测（三分支 + sentinel 证明 SDK 非零不触碰 itar + dilation_iters=0 guard + 4 wire 测含 PAI 逐字节等价断言）+ inceptio round-trip（`ROUNDTRIP OK`）+ 回读验证（10 台 per-cam nonzero 精确匹配）+ **接线实证 2026-07-08 inceptio 500 步 smoke（p02_wire_smoke）：6 训练相机全部 `[P0.2]` fallback 命中（coverage=front 1.91%/cross_L 11.11%/cross_R 10.34%/left 23.63%/right 25.74%/back 0.60%，dilate30 后口径）+ Training Complete + Mac 全套 960 passed** |
...
| **pinhole cuboid mask：behind-camera cuboid 空 mask、跨像平面不整列蔓延、正前方与修前字节等价**；FTheta 分支逐字节不动（既有 15 测不改全绿即证明） | v5-A5 ✅ (2026-07-02, 297a0bc) | `test_dynamic_mask.py` +8（含 `test_pinhole_fully_behind_camera_empty_mask` / `test_pinhole_fully_in_front_unchanged_by_z_filter` 112×112 精确块）+ `test_dynamic_mask_ftheta.py` 全绿 (Mac) |
| **cuboid ts 缺省模式逐字节等价 + interp 精度**：`pose_time_mode` 缺省输出与旧实现逐元素相等；40ms 相机偏移下 nearest 漂移 0.8m vs interp 位置误差 <1e-4 m、yaw <1e-5 rad；双覆盖取短弧；超区间容差外 inactive | v5-A2 ✅ (2026-07-02, 6983018) | `test_tracks_loader_interp.py` 11 测 + `test_tracks_loader_rotation.py` 14 测回归 (Mac)；wireframe 目检 f185 cross 相机 Δt=100ms 拖后→套准 |
| **非有限 ray 必修复且像素永久 invalid**：`pixels_to_camera_rays` 输出含 NaN/Inf 时（rational 畸变极点），修复为同行最近有限邻居 + ego valid mask=False；整行坏回退 +Z | v5-A1 ✅ (2026-07-02, 52224b3) | `test_repair_nonfinite_rays.py` 5 测 (Mac)；inceptio 实测 `[A1] camera_left_wide_90fov: repaired 1`（三次复现固定像素 (1917,1042) 坐实） |
| **loss 侧 where 掩蔽挡不住 0·NaN=NaN 梯度穿透**（训练必须丢帧而非 sanitize 的依据；torch 语义变更时此测试翻转提醒重审） | v5-A1 ✅ (2026-07-02, 794e269) | `test_nonfinite_guard.py::test_where_guard_cannot_stop_grad_poison_hence_train_drops_batch` (Mac)；inceptio sanitize 实验实证 1px→下帧全图 2073600px NaN |
| **MCMC 全死层 relocate 跳过不崩**（alive 空 → 空 multinomial CUDA invalid-configuration）+ **relocation kernel 坏行回退 donor**（非有限/非正 density/scale 不得经 log 逆激活写入参数） | v5-A1 ✅ (2026-07-02/03, 5f62cb0/0b960eb) | `test_layered_mcmc.py::test_relocate_skips_when_layer_fully_dead` + `test_mcmc_relocation_sanitize.py` 4 测 (Mac)；inc_b6a9 6-cam R1 过夜实测（bg 60% density NaN → 修复后重跑） |
