...
| **BUG-1 不变量** viser overlay 相机 ≡ backdrop 相机：同一 c2w 下「+Z 视线上的 world 点」必须投到 FTheta principal point（backdrop FTheta rays rz=cos>0 ⇒ 视线 = c2w +Z 列）；legacy `FLIP_VISER_TO_OPENCV` 必须看不见该点（回归绊线，防止误回退到 180° 镜像）。viewer 两处 compositor 构造必须传 `world_to_camera_flip=np.eye(4)`；FTheta overlay live 时 `add_line_segments` 必须跳过（防 pinhole 双画） | BUG-1 ✅ (2a4657e) | `test_viser_gui_4d_cuboid_overlay.py::test_viewer_overlay_camera_matches_renderer_camera` + 7 项行为合同 (Mac)；inceptio 30k ckpt 5 帧目视: bus 12m 框贴车身、box truck 框与 GT raw-camera 对照 (`validate_cuboid_7cam` flip=I) 同位同色；教训: 投影校准必须 GT 对照, 纯目视会被街道前后对称骗过 (B2_blended_clean.png 即 180° 镜像巧合) |
| **BUG-1b 不变量** cuboid label 与 wireframe 共投影：label 锚点（cuboid 顶角 vertex 7，`_cuboid_label_anchor` 单一来源）必须经同一 FTheta projector 投影（`PolylineLayerSpec.labels_world` → `OverlayLayer.texts`），背后/出 FOV 锚点剔除；FTheta 模式下浏览器端 3D labels 必须移除（防 pinhole 投影的文字漂离已对齐的框），pinhole ckpt 保留 3D labels | BUG-1b ✅ | `test_overlay_renderer.py` text ×3 + `test_viser_4d_ftheta_overlay_integration.py` label 投影/背后剔除 ×2 + `test_viser_gui_4d_cuboid_overlay.py` specs 含 label/移除 3D/pinhole 保留 ×3 (Mac)；inceptio verify 重跑: `t405 \| bus` 同色文字贴框顶角 |
| **BUG-1c 不变量** ego/track trajectories 与 backdrop 共投影：FTheta 模式下轨迹必须走 overlay（ego 绿 subdivide 3 / track per-class `class_color` 分层），`_add_ego_trajectory`/`_add_track_trajectories` 不得创建 pinhole 3D 轨迹（ego frustum 例外——位置 widget）；**"FTheta overlay (debug)" checkbox 已删除**，overlay 在 FTheta 模式无条件 live（`_overlay_compositor is not None` 即唯一判据），内容开关（show_cuboids/show_ego_traj/show_tracks）gate 各 overlay 层且切换必须 `need_update=True`（overlay 烤进背景图）。V3-VIZ.5 的「behind-camera 散射像素」= FLIP 镜像症状，flip=I 后 z>0/max_angle 剪除正常 | BUG-1c ✅ | `test_viser_gui_4d_cuboid_overlay.py` specs ego/track/toggles + FTheta 跳过 3D + pinhole 回归 ×7 (Mac 205 related passed)；inceptio verify: 路口车流轨迹贴路面弯曲（鱼眼平滑曲线）；ego 层 frame111 426/524 点可见聚于消失点 105×8px（Follow 视角自看轨迹的几何简并，非 bug） |
