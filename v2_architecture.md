...
| **T8.13 待修不变量** viz_4d.ego.primary_camera_intrinsics 必须存完整 fisheye/FTheta 内参 (多项式系数 + 畸变), viser_gui_4d 必须用 `Batch.intrinsics_FThetaCameraModelParameters` 而非 pinhole 简化, 否则 fisheye-trained ckpt 渲染乱糊不是 ground truth | T8.13 🟡 CONDITIONAL | **T8.12-FIX 诊断翻新**: 参考 repo `thinkpad:/home/yusun/repo/3dgrut/tools/viser_multilayer_nurec.py:280` 在同样 fisheye-trained 场景下用 `client.camera.fov = math.radians(90)` 硬设 + 纯 pinhole `make_rays` 渲染清晰, 反证 schema 扩 FTheta 不是必要前提. T8.13 仅在 T8.12-FIX vast.ai 视觉验收不达标时启动 |
| **T8.12-FIX 不变量** viser_gui_4d 初始 fov 必须显式设, 不能用 viser 默认 (~80°), 否则 fisheye-trained Gaussians 在 pinhole 投影下视觉退化 | T8.12-FIX 🟡 Mac | `viser_gui_4d.py` 加 3 CLI flags (`--initial_fov_deg` default=90 / `--camera_type` choices=[Pinhole,Fisheye] / `--camera_fov_deg`) + `_on_connect` & Reset View 显式 `client.camera.fov = math.radians(initial_fov_deg)` + 可选 `engine.camera_type = "Fisheye"` 走已存在 `_raygen_fisheye`. Mac NEW `test_viser_gui_4d_fov.py` 10/10 PASS + 189 passed/1 skip 0 回归. vast.ai 视觉待验 |
