...
| **T8.13 必走不变量** viz_4d.ego.primary_camera_intrinsics 必须存完整 FTheta 多项式系数 (`pixeldist_to_angle_poly` / `angle_to_pixeldist_poly` / `max_angle` / `linear_cde`), viser_gui_4d 必须用 `Batch.intrinsics_FThetaCameraModelParameters` 走 3dgut UT FTheta 分支, 否则 fisheye-trained ckpt 渲染乱糊不是 ground truth | T8.13 ⬜ 必走 | **T8.12-FIX vast.ai California 4090 实测确认**: Phase A.2 pinhole 任何 fov 都是 motion blur 隧道; Phase A.5 equirectangular fisheye 形态对但内容糊. 根因 = 3dgut UT rasterizer 用 intrinsics 投影 Gaussian 协方差到屏幕, FTheta-trained Gaussian 必须 FTheta intrinsics 才能正确投影 |
