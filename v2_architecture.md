...
| **T8.12 Stage 8** Pipeline 通 (87 FPS) + 修 2 真实 Stage 8 bug; **完整 Gaussian 渲染** 留 T8.13 (viz_4d schema 缺 fisheye 内参) | T8.12 ⚠️ RTX 4090 | vast.ai RTX 4090 24GB ($0.630/hr Norway). viser pipeline 端到端通 + scene primitives + timeline 同步 + dyn LiDAR 跟车 + 87 FPS, 但**Gaussian 视觉不匹配 render.py ground truth** 因 viz_4d schema 只存 fov_y_rad 没存 fisheye polynomial, viser pinhole 投影 fisheye-trained Gaussians 出乱糊. 修复留 T8.13 |
...
| **T8.13 待修不变量** viz_4d.ego.primary_camera_intrinsics 必须存完整 fisheye/FTheta 内参 (多项式系数 + 畸变), viser_gui_4d 必须用 `Batch.intrinsics_FThetaCameraModelParameters` 而非 pinhole 简化, 否则 fisheye-trained ckpt 渲染乱糊不是 ground truth | T8.13 ⬜ Backlog | 修改路径: `viz/metadata.py:_extract_ego` 抽完整内参 → `viz/inject.py` 同步 → `viser_gui_4d.update()` 用 FTheta intrinsics, 可能需要 fisheye ray gen 替代 kaolin pinhole. 预估 0.5-1 d |
