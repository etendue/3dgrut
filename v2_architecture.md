...
| **BUG-1b 不变量** cuboid label 与 wireframe 共投影：label 锚点（cuboid 顶角 vertex 7，`_cuboid_label_anchor` 单一来源）必须经同一 FTheta projector 投影（`PolylineLayerSpec.labels_world` → `OverlayLayer.texts`），背后/出 FOV 锚点剔除；FTheta overlay live 时浏览器端 3D labels 必须移除（防 pinhole 投影的文字漂离已对齐的框），overlay OFF 回退 3D labels | BUG-1b ✅ | `test_overlay_renderer.py` text ×3 + `test_viser_4d_ftheta_overlay_integration.py` label 投影/背后剔除 ×2 + `test_viser_gui_4d_cuboid_overlay.py` specs 含 label/移除 3D/回退 ×3 (Mac)；inceptio verify 重跑: `t405 \| bus` 同色文字贴框顶角 |
