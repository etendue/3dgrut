...
authoritative runbook lives in **[CLAUDE.md](../../../CLAUDE.md) → "Vast.ai 远程执行环境"**
...
`~/.ssh/config` alias `vast-rtx4090` yourself (Python snippet in CLAUDE.md).
...
```
(Run as a background/long task; full keypair + config steps in CLAUDE.md.)
...
```
Use the standard multilayer command (see CLAUDE.md "训练配置约定"): `--config-name
...
(see CLAUDE.md "num_workers 必须按系统内存调"). Keep `use_lidar_depth` as the run needs;
...
Per CLAUDE.md discipline: write the real per-class PSNR / LPIPS / commit hash / it·s
