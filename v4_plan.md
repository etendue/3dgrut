...
kanban
    ...
    "Done"
        [E2.8 系统性 rigid 全替流水线 ✅ 2026-06-17 core：USDZ拆→全 vehicle track 换 AH 资产→QA 闸，端到端实测 coverage=1.0、8/8 车全替（含 heavy_truck）、QA passed、viser 加载渲染通过；viz_4d torch bug 修复；Task6 定量NTA/FID + Task7 建库待 GPU]
...
| **E2.8** ★ | E2 | **系统性 dynamic rigid 全替流水线（单 clip scene factory）**：E2.5 手术式换 3 车 → 系统化全 vehicle track 成批换 AH 资产库（class+size 最近 + fallback ladder，一资产可服务多 track）；USDZ拆（deformable 丢弃）→ 全替 → harmonizer 协调 → QA 闸（sanity + NTA-IoU/FID）。新增 `asset_bank.py`（bank 查询）/`e28_replace.py`（全 track 枚举+批 align+守护）/`nre_usdz_viz4d.py`（USDZ→可渲染 ckpt+viz_4d，移植 fervent-knuth rig/viz4d 接线到 checkpoint.ckpt flavor）/`scripts/e28_systematic_replace_pipeline.py`（driver）；复用 E2.5 e25_inject/warmstart_*、E2.7 nre_usdz_loader、E2.1/E2.6 harmonizer、E1.2 NTA、E1.4 FID | spec [`2026-06-17-e28-...-design.md`](docs/superpowers/specs/2026-06-17-e28-systematic-rigid-replacement-pipeline-design.md) + plan（E2.5 升级）| 2.5 | 🟡 core ✅ | **2026-06-17 core 完成**（6144f23 vendor E2.5 / 946147c bank / 60103e6 assign / 8dc4d0a replace / cd02856+05e8474 qa / 321926f driver）：**端到端 inceptio 实测 coverage=1.000、8/8 vehicle track 全替（含 heavy_truck，is_vehicle 改子串）、QA passed（opacity floor 实测重标定 0.15→0.02）、viser 加载渲染通过（179 tracks）**；22 E2.8 测试全绿 + E2.5 自带 17 绿。**viser 实测抓+修真 bug**：viz_4d tracks numpy→torch（engine/render populate_tracks auto-hook 需 .to()）。**待 GPU**：Task6 定量 NTA-IoU/FID + harmonizer 协调、Task7 asset-harvester 建 5 类库 | gate=E2.1+E2.5+E2.6+E1.2/E1.4 ✅；验收=coverage 100% + NTA≥recon + FID after<before + viser 目测干净 |
...
| **E2** | 生成修复链（NuRec 思路移植）+ 编辑协调 spike + viser temporal 后处理 + viser USDZ 视觉对标 + dyn rigids 接线 + 系统性全替流水线 | 6/10（含 1 备选；E2.8 core ✅ 定量待 GPU） | 同左；**E2.8 🟡 core ✅ 系统性全替：USDZ拆→8/8 车全替 coverage=1.0→QA passed→viser 渲染通过（22 测绿；定量 NTA/FID + 建库待 GPU）**；**E2.1 ✅ 离线 Harmonizer：FID −30%/KID −60%/NTA +35%**；**E2.5 ✅ 目测 spike：harmonizer 协调有效但有限（违和感降低、未完全自然）**；**E2.6 ✅ inceptio 目测通过（V=5 temporal）**；**E2.7 ✅ viser_gui_4d 加载 NVIDIA usdz：路面横向 3m/6m + 360° 不退化**；**E2.7-B ✅ dynamic_rigids 接线（cuboid wireframes + dyn gaussian 位置贴车随 timeline 动，commit 7e5edac；颜色烟雾感转 E2.7-C）**；E2.7-C ⬜ dyn features_albedo Fourier→SH | cc ≥ 24.7 / grad_corr 0.744 不退 | 🟡 |
