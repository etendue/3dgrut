...
| **E3.3** ★ | E3 | **BEV 纹理平面化**（v4 backlog 转正）：road 颜色不再 per-gaussian SH，改 BEV feature grid/纹理图采样、真正贴在高度场平面 → **外推天然正确**（参数化级根治 aperture problem；ExtraGS Road Surface Gaussians 同思路）；复用 [`road_region.py`](threedgrut/model/road_region.py) BEV 网格基建 | v3 § 5 backlog「外推终极方向」 | 3 | ⏭ | **实测判负（2026-06-23）**：6k A/B on(bevroad) vs off(multilayer) BEV 全面略差——cc 23.49/23.74 · lane 3m 0.372/0.380 · 6m 0.282/0.307 · road_crop 26.56/27.35；viser 视觉确认无改善、白条仍 bg。**印证 spec §1.4**：road 弱所有权（opacity~0.014、bg 兜底）下颜色参数化无效（改的层不主导渲染 + 1m grid 损细节）。**代码在 [PR #37](https://github.com/etendue/3dgrut/pull/37)（opt-in OFF、不进 main；grid optimizer + ckpt round-trip 基建保留）**，待 E3.6 白条 takeover 解决后或可重试。 |
...
- **2026-06-23（晚）road 外推三方收口：E3.2.5→baseline / E3.3 BEV 判负 / E3.2.6 takeover 三档否（大g 决策，baseline 改动同本 commit）**。承 E3.3 BEV spike 负结果 + E3.2.6 takeover 三档实测，大g 拍板 road 外推方向 + baseline 配方。
  - **E3.2.5 roaddisk 几何 bundle 默认进 baseline**（`configs/apps/ncore_3dgut_mcmc_multilayer.yaml`）：6k A/B on>off（cc 24.01 vs 23.60 **+0.41** / lateral lane grad_corr +0.057@3m +0.039@6m / freeze 铁证 rot 0°·z 1mm·N 恒定）→ multilayer.yaml 默认含 `road_init_knn_k=5` + `scale_z_max=0.001` + `positions_lr=1e-6` + `freeze_rotation_grad=true` + `strategy.exclude_layer_ids=[road]`（原 roaddisk preset 降冗余）。compose 验证 road override 生效 + 全套 919 测零回归 + takeover penalty 保持 λ0.1/z_band0.4 现状。
  - **E3.3 BEV 纹理平面化 ⏭ 实测判负**：Task0/1 代码链路全通（fused_view pre-bake + grid optimizer + ckpt round-trip，G1 grid-trains gate PASS）。6k A/B on(bevroad) vs off(multilayer) **BEV 全面略差**——cc 23.49/23.74 · lane 3m 0.372/0.380 · 6m 0.282/0.307 · road_crop_psnr 26.56/27.35；viser 视觉确认无改善、白条仍 bg。**印证 spec §1.4**：road 弱所有权（opacity~0.014、bg 兜底）下颜色参数化无效（改的层不主导渲染 + 1m grid 损细节）。**代码独立留 [PR #37](https://github.com/etendue/3dgrut/pull/37)（opt-in OFF、不进 main；grid optimizer + ckpt round-trip 基建保留）**，待 E3.6 白条 takeover 解决后或可重试。
  - **E3.2.6 takeover ⏭ 三档否**：调强 bg_road_penalty 压 road 区 bg，λ0.1/0.2/0.4 三档实测——白条均没收回 road（viser）+ 守护线 cc 噪声到负（λ0.1 24.01 / λ0.2 23.52 退 −0.49 / λ0.4 23.78，vs multilayer 23.74）。「压 bg opacity」路否定、不进 baseline。
