...
| **E3** | 表示侧外推强化（与 E2 互补） | 1/5（含 1 备选） | 同 E2 验收口径；E3 减伪影产生、E2 修残余；**E3.2.5 ✅ spike 几何硬退化 disk：6k A/B on>off（cc +0.41 / lateral lane grad_corr +0.04~0.06 / freeze 铁证 rot 0°·z 1mm·N 恒定）→ 反驳 roadoff「光冻结变差」；viser 视觉确认 on 斑马线分明，发现白条被 bg 抢 → E3.2.6 takeover 调强 follow-up；E3.3 代码层 Task0/1 ✅（BEV pre-bake，20 测）** | 同上 | 🟡 |
