...
| **E2** | 生成修复链（NuRec 思路移植）+ 编辑协调 spike + viser temporal 后处理 + viser USDZ 视觉对标 | 3/7（含 1 备选） | 同左；**E2.1 ✅ 离线 Harmonizer：FID −30%/KID −60%/NTA +35%，lane_grad_corr 退化待 E2.2 蒸馏修 → E2.2 GO**；E2.5 插入协调；**E2.6 ✅ inceptio 目测通过（V=5 temporal，延迟 ~1000ms；Conv3d warmup 修复 cce14ba）**；**E2.7 ✅ viser_gui_4d 加载 NVIDIA usdz（world_to_nre +38m 对齐 + frustum/Reset View 修复 + JIT FileBaton stale lock 清理）：视觉对标 NRE 路面横向 3m/6m + 360° 不退化，直观证实 E2.2/E3.3 配方方向** | cc ≥ 24.7 / grad_corr 0.744 不退 | 🟡 |
