...
| **E2** | 生成修复链（NuRec 思路移植）+ 编辑协调 spike + viser temporal 后处理 | 2/6（含 1 备选） | 同左；**E2.1 ✅ 离线 Harmonizer：FID −30%/KID −60%/NTA +35%，lane_grad_corr 退化待 E2.2 蒸馏修 → E2.2 GO**；E2.5 插入协调；**E2.6 🟡 代码完成（36/36 Mac 测零回归）、inceptio 端到端验证 pending 等 β' 释放 GPU** | cc ≥ 24.7 / grad_corr 0.744 不退 | 🟡 |
...
- **2026-06-15 E2.6 代码完成，端到端验证 pending（等 β' 释放 GPU）**（branch `e26-harmonizer-temporal`；commit 链 `bff0ef9`(worktree)→`9de9101`(IPC 链路)→`5cf26e7`(viser 接线)→`4058704`(demo 脚本)）——把 viser_gui_4d 的单帧 DiFix 后处理扩展为 Harmonizer temporal 模式（V=1+K，回读前 K 帧已修复输出做时序参考），用于 Play 连续序列去闪烁：
  ...
  - **端到端验证 pending（Step 4b）**：inceptio worktree `~/repo/3dgrut2-wt/e26` @ 4058704 就绪（submodule 补齐）。卡点：β' 训练占 GPU ~10.5GB，temporal server(~8GB)+渲染(~4GB) 并发撞 24GB OOM 会杀 β' → 按 plan 风险表错峰，等 β' 完成后起 temporal server + 跑 demo + 目视存档（raw vs nontemporal vs temporal 三列去闪烁对照）。验证结果回来后回填本行 + 看板 E2.6 ⬜→✅。
