...
- Both resolved configs explicitly freeze `dataset.mask_forward_invalid_pixels=true` and `dataset.opencv_pinhole_use_validity_domain=false`. The mask applies in Arm P; in Arm F it is a strict no-op because dataset masking is guarded by `isinstance(camera_model, OpenCVPinholeCameraModel)`. Keeping both settings identical preserves the single-variable A/B and avoids the invalid mask-plus-validity-domain combination.
...
- Smoke must show samples from all seven selected cameras, finite rays/loss, no model fallback, all seven metadata entries, both final training and test tables, and complete per-camera metric keys.
...
The corrected 2026-07-17 native-resolution **nine-camera survey** remains **STOP** as a nine-camera representation: seven cameras pass and two fail. `camera_front_standard_55fov` fails the non-radial and full-image angular gates; the recorded `camera_front_tele_30fov` max-endpoint result is retained in the survey. The seven wide/rear cameras have expected OpenCV physical-domain coverage of `62.8745%–64.5493%` under the production `0.8<icD<1.2` contract; later low-residual rational roots are explicitly invalid. The user-approved scope revision permits GPU work only for the matched seven-camera subset. See `PIN_FTHETA_9CAM_PARAMETER_SURVEY.md`, `scripts/pin_ftheta_b6a9_9cam_params.json`, and the strict training artifact `scripts/pin_ftheta_b6a9_7cam_params.json`.
