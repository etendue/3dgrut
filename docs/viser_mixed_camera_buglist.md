...
| MC-6 | 非 FTheta distorted pinhole overlay 投影公式 | ✅ 已关闭 | P1 | `PinholeForwardProjector` 已改为 rational numerator/denominator + tangential + thin-prism + SDK validity gate；inceptio 5 相机 NCore SDK parity 100% validity agreement、0.000017–0.000036 px MAE、integer roundtrip verified |
...
| Phase | 状态 | 证据 |
|---|---|---|
| A OpenCV rational formula RED tests | ✅ | 4 个 RED 测试验证 rational 分母、thin-prism、trust gate |
| B 实现修正 | ✅ | rational 公式 + 信任门 0.8<icD<1.2 + 系数补齐 |
| C NCore SDK parity validator | ✅ | 5 相机 inceptio 验证：100% validity agreement，MAE 0.000017–0.000036 px，integer roundtrip verified |
| D 文档证据 | ✅ | `docs/T8_artifacts/C4_opencv_rational_overlay_validation.md` |
