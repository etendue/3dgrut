...
**一句话脉络**：先在 Mac 上把「车道线锐度指标」这套纯张量工具（Task 1–3）+ 把它接进 eval 数据链路（Task 4–5）写好并单测通过（Task 6 是上 GPU 前的 Mac 关卡）——全部离线、便宜；再上 GPU 自跑 Mapillary 模型产出 lane GT、在 baseline 上跑出真实数字立锚（Task 7–8）；最后回填文档（Task 9）。

| # | Task | 为什么做（why） | 目标（done 标准） | 机器 |
|---|---|---|---|---|
| 1 | `dilate_mask` 膨胀工具 | 车道线是亚像素~几像素的**细线**，直接在细 mask 上算 LPIPS/PSNR 会被面积主导而失效（P0 已坐实）→ 必须先把细线膨胀成有宽度的 band，指标才有意义 | 纯 torch 膨胀函数（细 mask → N px band） | Mac |
| 2 | `grad-corr` 锐度标量 | 车道线质量的核心是「线锐不锐、位置对不对」，PSNR/LPIPS 都间接；edge-IoU 要标定阈值且对细线脆弱 → 用**阈值无关**的梯度相关直接量边缘锐度 | Sobel 梯度幅值的 Pearson 相关函数 | Mac |
| 3 | `compute_lane_metrics` 主函数 | 把膨胀+锐度+LPIPS+PSNR 组装成一个 lane 指标入口；spec 要求「一次报多个候选，baseline 实测后再挑主指标」 | 给 pred/gt/lane mask → 返回 4 个候选指标 | Mac |
| 4 | datasetNcore 加载 lane 产物 | lane 类来自**独立** Mapillary 产物（不能动现有 Cityscapes `semantic_sseg`，否则扰动 person/road_crop）；lane 纯 eval 用 → 只在 val/test 路径加载（P0.4 双路径教训） | val/test 加载 `*.aux.lane.zarr.itar` → `semantic_lane_sseg` | Mac |
| 5 | render.py 接 lane eval + metrics.json | 把指标接进 eval loop、写进 metrics.json、限前视相机；**无 lane 产物时必须字节等价**（不破坏现有 eval / 其他数据集） | `render_all` 前视 gate → `mean_lane_*` 字段 | Mac |
| 6 | Mac 全门回归 | CLAUDE.md 纪律：单测便宜、GPU 贵 → 上 GPU 前先把所有 Mac 单测跑绿，防带 bug 上去浪费 GPU 时间 | 受影响模块全测绿 + 无 import 副作用 | Mac |
| 7 | 生成 lane 产物 + 对账 id | 现有 sseg **根本没有 lane 类** → 必须自跑 Mapillary 模型产出 lane GT，并读真实输出对账 lane 的 class id（占位 (24,) 改实值） | `*.aux.lane.zarr.itar` + `LANE_CLASS_IDS` 定值 | inceptio |
| 8 | baseline 立锚 + 零回归 | **「测量门」的本质**：在现 baseline 上跑出真实 lane 数字，作为 P3.1/P3.2 改善的对照基线；并证明本次改动纯增量（不动其他指标） | metrics.json 出 `mean_lane_*`、grad_corr 立锚、守护线零回归 | inceptio |
| 9 | 文档同步 | CLAUDE.md 纪律：不同步文档 = 任务未完成；看板与实际脱节会让后续任务依赖判断出错 | 回填 v3_plan_revised + v2_architecture | Mac |

> **怎么读本 plan**：上表是「**为什么 + 目标**」（建立全局）；下面每个 `## Task N` 是「**怎么做**」（TDD 逐步 + 完整代码 + 验证命令）。先扫上表，再按需翻具体 Task 的实现步骤。每个 Task 标题下也补了一行「为什么」做即时提醒。
...
> **为什么**：车道线是几像素宽的细线，直接在细 mask 上算 LPIPS/PSNR 会被面积主导失效（P0 坑）→ 先做一个膨胀工具，把细线扩成有宽度的 band，后面的指标才站得住。**目标**：纯 torch 膨胀函数。
...
> **为什么**：车道线质量的核心就是「线锐不锐、在不在对的位置」，但 PSNR/LPIPS 都只能间接反映；edge-IoU 要标定梯度阈值又对细线脆弱 → 用阈值无关的梯度相关直接量。**目标**：Sobel 梯度幅值的 Pearson 相关标量。
...
> **为什么**：把 Task 1 膨胀 + Task 2 锐度 + 现成 LPIPS/PSNR 组装成**一个** lane 指标入口；spec 要求「一次报多个候选，baseline 实测后再挑主指标」，所以这里同时产 4 个候选。**目标**：给 pred/gt/lane mask → 返回 4 候选指标的函数。
...
> **为什么**：指标有了，但 lane 类来自一个**独立**的 Mapillary 产物——不能动现有 Cityscapes `semantic_sseg`（一动就扰动 person/road_crop 指标，spec 禁止）。lane 又只在 eval 用，所以只在 val/test 数据路径加载（呼应 P0.4「train 有 val 漏」的双路径教训）。**目标**：val/test 把 `*.aux.lane.zarr.itar` 读进 `semantic_lane_sseg` 透传给评测器。
...
> **为什么**：数据进来了，还得把指标真正接进 eval loop、写成 metrics.json 字段，并限定在前视相机（lane 最清处）。**硬约束**：没有 lane 产物时 metrics.json 必须字节等价——否则会破坏所有不带 lane 的现有 eval / 其他数据集。**目标**：`render_all` 前视 gate 调用 → `mean_lane_*` 字段。
...
> **为什么**：上 GPU 前的最后一道 Mac 关卡。CLAUDE.md 纪律是「单测便宜、GPU 贵」——带 bug 上 GPU 跑一次就是几十分钟 + GPU 时间的双倍浪费。**目标**：受影响模块单测全绿 + 确认 `per_class_eval` 无 cv2/NCore import 副作用。
...
> **为什么**：到这里 Mac 侧的指标 + 接线都好了，但**现有 sseg 根本没有 lane 类**（Cityscapes 19 类里 road 是粗类）。所以必须自跑一个带 lane 类的 Mapillary 模型，逐帧产出 lane GT 产物，再读它的真实输出对账 lane 的 class id（把占位 (24,) 改成实测值）。**目标**：`*.aux.lane.zarr.itar` + `LANE_CLASS_IDS` 定值。
...
> **为什么**：这是「**测量门**」的核心动作——在现 baseline ckpt 上跑出真实 lane 数字，作为 P3.1/P3.2 改善的**对照基线**（没有这个锚点，后面任何「改善」都不可证）；同时核对守护线零回归，证明本次改动是纯增量、没碰其他指标。**目标**：metrics.json 出 `mean_lane_*`、grad_corr 立锚、守护线零回归。
...
> **为什么**：CLAUDE.md 纪律——代码跑通但文档不同步，会让看板与实际状态脱节，导致后续任务的依赖判断出错。把「更新 plan/architecture」当作 task 的最后一步。**目标**：回填 `v3_plan_revised.md`（看板/gap 表/Done Log）+ `v2_architecture.md`（文件清单/不变量）。
