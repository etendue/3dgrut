...
## 4cab 大卡车 demo + 30k / pose_adjustment 实验（2026-06-29/30，inceptio）

**目标（用户决策）**：inceptio 的 4cabad44 clip 实际只有 1 辆大卡车，demo 只需把这辆卡车的 trajectory 塞进 NCore shard。

### 数据流 + 做法
- `aux.lidar-sseg` = 相机分割投影到 LiDAR 点的**逐点 class**；聚类带 car/truck/bus 标签的 LiDAR 点 → cluster 中心 = position（**class 来自相机、position 来自 LiDAR**）。init 只需 position（`|local|≤size/2` 裁剪，朝向错也能裁进点）。
- **single-track 模式**（CLI `--single-track`）：密集多车场景 tracking 易碎（4cab 同一卡车被切成 8 段）；单车 demo 改为每帧取点最多 cluster 串 1 条 trajectory + 中心移动平均平滑 + 朝向取**运动方向**（替代单边 LiDAR 几何 yaw）。
- 4cab 实测：1 heavy_truck track 17.8×2.7m / 43 obs → `init_dynamic_rigid_layer` 5000 particles。

### 训练 + 渲染
- **1k smoke**：卡车 actor init 5000 粒子，训练不崩。
- **30k baseline**：psnr 25.33 / psnr_masked 25.78；`render.py`（绕开 yolov8 eval + viewer road config bug）显示卡车 actor = **贴路面的模糊红色斑块，没收敛成卡车形状、位置未对上 GT**。
- **30k + pose_adjustment**（enabled / lr_rot 1e-5 / lambda_t 1e-2 / lambda_r 1e-1）：`🛞 LearnablePose: 1 track` 生效；但 render 对比图与 baseline **几乎一致**——卡车 actor 仍是模糊红团，psnr_masked 25.70（略低）。

### 诚实结论（demo 天花板）
- **pipeline 端到端通 = demo 核心达成**：纯 LiDAR → 卡车 trajectory → NCore shard → dynamic_rigids init → 训练渲染不崩。
- **但「清晰动态卡车重建」受限于 cuboid 精度**：单边/稀疏 LiDAR 聚类的 cuboid 位置/朝向太粗（红团位置压根不在 GT 卡车处），`pose_adjustment` 是局部小修 + temporal smooth，**从这么错的 init 爬不出来**（lr_rot 1e-5 / 30k 无改善）。这是单视角 LiDAR + 纯几何 cuboid 的真实天花板，非参数问题。
- **真正改进方向（未做）**：提高 cuboid init 精度 —— 车类先验尺寸 + 从可见面外推中心（修单边回波中心偏），让 actor 位置先对上 GT，再训练。

### 产物（4cab）
- shard：inceptio `~/work/4cab_autocuboids/autocuboids.ncore4-auto_cuboids.zarr.itar`。
- ckpt：`~/work/output/4cab_cuboid_30k{,_poseopt}/inceptio_4cabad44-.../ours_30000/ckpt_30000.pt`。
- 目测：`/tmp/4cab_poseopt_compare.png`（render vs GT）/ `scripts/spikes/viz_truck_traj.py`（卡车轨迹）。
