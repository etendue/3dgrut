...
def _draw_polyline(pil_img: Image.Image, uv: np.ndarray, vis: np.ndarray, color: tuple, width: int = 2) -> int:
    ...
    for i in range(uv.shape[0] - 1):
        if bool(vis[i]) and bool(vis[i + 1]):
            draw.line(
                [(float(uv[i, 0]), float(uv[i, 1])), (float(uv[i + 1, 0]), float(uv[i + 1, 1]))],
                fill=color,
                width=width,
            )
            drawn += 1
...
def _draw_cuboids_on(
    ...
    for tid, edges, color in cuboid_set:
        ...
        results = projector.project_polylines(polylines, c2w_opencv, subdivide_n=subdivide_n)
        segs = 0
...
def main(argv=None) -> int:
    ...
    p.add_argument("--path", required=True, type=str, help="NCore manifest path")
    p.add_argument("--ckpt", required=True, type=str)
    p.add_argument("--frame_idx", type=int, default=20, help="primary-camera frame index (default 20)")
    ...
    print(f"[compare] loading dataset via Hydra config={args.config_name}", flush=True)
    ...
    if train_indices is None or len(train_indices) <= args.frame_idx:
        print(
            f"[compare] ERROR: frame_idx {args.frame_idx} out of range "
    ...
    print(f"[compare] frame_idx={args.frame_idx} → cam_frame_idx={cam_frame_idx}  " f"t_us={t_us}", flush=True)
    ...
    if intrinsics_result is None:
        print("[compare] ERROR: cannot extract intrinsics for primary camera", file=sys.stderr)
        return 4
    ftheta_dict, model_type_name = intrinsics_result
    if "FTheta" not in model_type_name:
        print(f"[compare] ERROR: primary camera is not FTheta " f"(model={model_type_name})", file=sys.stderr)
        return 5
    # NCore _get_camera_model_parameters_for_resolution already returns the
    # 8-key dict expected by FthetaForwardProjector.
    projector = FthetaForwardProjector(ftheta_dict, world_to_camera_flip=np.eye(4))  # OpenCV in, no flip

    cuboid_set = _collect_active_cuboid_edges(meta, t_us)
    print(f"[compare] active cuboids @ t_us: {len(cuboid_set)}", flush=True)

    n_drawn_raw = _draw_cuboids_on(pil_raw, projector, T_c2w_opencv, cuboid_set)
    _draw_caption(
        pil_raw, f"RAW + cuboid  |  frame {args.frame_idx}  |  " f"{n_drawn_raw}/{len(cuboid_set)} cuboids drawn"
    )
    ...
    ftheta_for_engine = (
        meta.ego_primary_intrinsics_ftheta if meta.ego_primary_intrinsics_ftheta is not None else ftheta_dict
    )
    print(f"[compare] engine fisheye_intrinsics keys: " f"{sorted(ftheta_for_engine.keys())}", flush=True)
    ...
    n_drawn_recon = _draw_cuboids_on(pil_recon, projector, T_c2w_opencv, cuboid_set)
    _draw_caption(
        pil_recon, f"RECON + cuboid  |  frame {args.frame_idx}  |  " f"{n_drawn_recon}/{len(cuboid_set)} cuboids drawn"
    )
    ...
    combined = Image.new("RGB", (pil_raw.width, pil_raw.height * 2 + 8), color=(20, 20, 22))
