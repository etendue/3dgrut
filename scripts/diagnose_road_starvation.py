...
def _binned_dead_fraction(dist: np.ndarray, dead: np.ndarray, edges) -> list[dict]:
    ...
    for lo, hi in zip(edges[:-1], edges[1:]):
        ...
        rows.append({"lo": float(lo), "hi": (float(hi) if np.isfinite(hi) else None), "n": n, "dead_frac": df})
    return rows
    ...
    *,
    ...
    out: dict = {
        ...
        "T1": {
            ...
            "n_dead": int(dead.sum()),
            "opacity_pct": {
                q: float(np.percentile(road_opacity, p)) for q, p in (("p10", 10), ("p50", 50), ("p90", 90))
            },
        },
    }
...
def main(argv=None) -> int:
    from scripts.diagnose_road_bev_holes import (
        _ego_xy_from_ckpt,
        _extract_layer_xy_opacity,
    )
    ...
    print(
        f"[T1] road N={s['n_particles']}  dead(op<={args.dead_thresh})={s['T1']['n_dead']} "
    ...
    print(
        f"     dead ego-dist p50={s['T2']['dead_ego_dist_p50']:.1f}m  vs  alive p50={s['T2']['alive_ego_dist_p50']:.1f}m"
    )
    ...
    if "T3" in s:
        ...
        for r in s["T3"]["lidar_dist_bins"]:
            ...
            print(f"     {label:>14}{r['n']:>10}{r['dead_frac']*100:>10.1f}%")
        print(
            f"     dead lidar-dist p50={s['T3']['dead_lidar_dist_p50']:.2f}m p90={s['T3']['dead_lidar_dist_p90']:.2f}m"
