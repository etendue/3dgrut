...
from threedgrut.datasets.tracks_loader import (  # noqa: E402
    load_tracks_from_ncore_cuboids,
)
from threedgrut_playground.utils.cuboid import (  # noqa: E402
    cuboid_world_edges,
    instance_color,
)
...
def _edges_at_frame(tracks: dict, frame_idx: int) -> list[tuple[str, np.ndarray, tuple]]:
...
def main(argv=None) -> int:
    ...
    p.add_argument("--config_name", required=True, help="Hydra config (e.g. apps/ncore_3dgut_mcmc_multilayer)")
    p.add_argument("--path", required=True, help="NCore manifest json path")
    p.add_argument(
        "--camera_ids", nargs="+", required=True, help="cameras to render, first one drives the frame sampling"
    )
    p.add_argument(
        "--class_filter", nargs="+", default=None, help="cuboid classes to keep (default: automobile/heavy_truck/bus)"
    )
    p.add_argument("--n_frames", type=int, default=4, help="how many evenly-spaced reference frames to sample")
    p.add_argument(
        "--ts_mode",
        default="ref_nearest",
    ...
    ref_ts_end = np.asarray(ref_sensor.frames_timestamps_us[:, _nd.FrameTimepoint.END], dtype=np.int64)
    ...
    if args.ts_mode == "per_camera_interp":
        timeline = np.asarray(build_cuboid_frame_timeline_us(dataset, args.ts_mode), dtype=np.int64)
    else:
        timeline = ref_ts_end  # legacy: unwindowed ref-camera END timestamps
    tracks = load_tracks_from_ncore_cuboids(loader, timeline, pose_time_mode=CUBOID_TS_MODES[args.ts_mode], **kw)
    classes = sorted({t["class"] for t in tracks.values()})
    n_active = sum(int(t["frame_info"].sum()) for t in tracks.values())
    print(
        f"[pretrain] tracks={len(tracks)} classes={classes} " f"total_active_(track,frame)={n_active} ref_frames={F}",
        flush=True,
    )
    if not tracks:
        print("[pretrain] WARNING: no cuboids after class filter — nothing to draw.", flush=True)
    ...
    for fi in idxs:
        ...
        for cam_id in args.camera_ids:
            if args.ts_mode == "per_camera_interp":
                ...
                ts_c = np.asarray(sensor_c.frames_timestamps_us[:, _nd.FrameTimepoint.END], dtype=np.int64)
                tc = int(ts_c[int(np.argmin(np.abs(ts_c - t_us)))])
                uidx = int(np.clip(np.searchsorted(timeline, tc), 0, len(timeline) - 1))
            ...
            try:
                ...
                pil, st = blk, {"model": "error", "drawn_cuboids": 0, "total_cuboids": len(edges_cam), "drift_us": -1}
            panels.append(pil)
            print(
                f"[pretrain] f{fi:03d} t={t_us} {cam_id:>32s} "
                f"model={st['model']:<9s} drawn={st['drawn_cuboids']}/{st['total_cuboids']} "
                f"dt={st['drift_us']/1000:.1f}ms",
                flush=True,
            )
