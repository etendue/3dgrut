#!/usr/bin/env bash
# Native-FTheta six-camera formal run: full 20-second window, 30k iterations.
...
RUN_BASE="${RUN_BASE:-$HOME/work/output/pin_ftheta_native_6cam_full}"
ARMS="${ARMS:-F}"
...
"$PYTHON_BIN" -m scripts.pin_ftheta_native_6cam_validation \
  --mode full --pin-manifest "$PIN_DATA_PATH" --ftheta-manifest "$FTHETA_DATA_PATH" \
...
run_arm() {
  local arm="$1"
  local data_path="$2"
  local name="pin_ftheta_native_6cam_full_arm${arm}"
