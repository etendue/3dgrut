#!/bin/bash
# FTheta v4 matched seven-camera P/F full run: full 20 seconds, 30k steps.
...
RUN_BASE="${RUN_BASE:-$HOME/work/output/pin_ftheta_v4_full_ab_runs}"
...
DRIVER_FILE="scripts/pin_ftheta_7cam_v4_full_ab.sh"
...
"$PYTHON_BIN" -m "$PREFLIGHT_VALIDATOR_MODULE" preflight \
  --mode full --repo-root "$REPO_DIR" --driver "$DRIVER_FILE" \
  --config-name "$CONFIG_NAME" --artifact "$FTHETA_PARAMS" \
  --input-manifest "$DATA_PATH" --run-base "$RUN_BASE" \
  --expected-commit "$EXPECTED_COMMIT"

if [ "$MODE" = "--preflight" ]; then
  echo "=== PIN-FTHETA v4 full A/B preflight passed (CPU-only; no output created) ==="
  exit 0
fi
...
on_error() {
  ...
  echo "ERROR: PIN-FTHETA v4 full A/B failed at stage=$CURRENT_STAGE rc=$rc manifest=$RUN_MANIFEST" >&2
  exit "$rc"
}
trap on_error ERR

exec > >(tee "$RUN_ROOT/driver.log") 2>&1
echo "=== PIN-FTHETA v4 immutable full-run root: $RUN_ROOT ==="
...
run_arm() {
  ...
  local name="pin_ftheta_v4_7cam_arm${arm}_20s_30k"
  ...
  echo "=== PIN-FTHETA v4 full Arm $arm train start: $name $(date '+%F %T') ==="
  ...
  echo "=== PIN-FTHETA v4 full Arm $arm native render: $checkpoint $(date '+%F %T') ==="
  ...
  echo "=== PIN-FTHETA v4 full Arm $arm done $(date '+%F %T') ==="
}
...
echo "=== PIN-FTHETA v4 matched full A/B complete $(date '+%F %T') manifest=$RUN_MANIFEST ==="
