...
def per_cuboid_counts_owner_aware(
    ...
    for tid, size in tracks_size.items():
        ...
        outlier_dist = float((owned_abs - size_half).clamp(min=0.0).max().item()) if n > 0 else 0.0
...
def _load_layered_model_from_ckpt(ckpt_path: Path):
    ...
    if conf is None:
        raise RuntimeError(
            "ckpt has no 'config' nor per-layer fallback — diagnostic requires " "a v2 LayeredGaussians ckpt."
        )
    ...
    if not isinstance(viz_4d, dict) or "tracks" not in viz_4d:
        raise RuntimeError("ckpt has no viz_4d.tracks block — re-run with an injected ckpt.")
...
def _format_console(report: dict) -> str:
    ...
    lines = [
        ...
        f"  alive (opacity > {report['opacity_threshold']:.4f})  : " f"{s['alive_total']} ({s['alive_pct']:.2f} %)",
...
def diagnose(ckpt_path: Path, opacity_threshold: float = 0.005) -> dict:
    ...
    if "dynamic_rigids" not in layers:
        raise RuntimeError(
            "ckpt's LayeredGaussians has no 'dynamic_rigids' layer — nothing " "to diagnose on the dyn side."
        )
    ...
    if path == "owner":
        ...
        centers_pct = 100.0 * (accounted - out_of_total) / accounted if accounted > 0 else None
    else:
        opacity = torch.sigmoid(densities_raw.view(-1))
        alive_total = int((opacity > opacity_threshold).sum().item())
        zero = 0
