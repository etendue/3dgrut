...
def _hist_named(counts: np.ndarray, classes: list, top: int = 8) -> list:
    ...
    for cid in np.argsort(-counts):
        if counts[cid] == 0 or len(out) >= top:
            break
        name = "ignore" if cid == IGNORE else (classes[cid] if cid < len(classes) else f"class_{cid}")
        out.append(
            {
                "class_id": int(cid),
                "name": str(name),
...
def stage_hist(clip_dir: Path) -> dict:
    ...
    report = {
        ...
        "camvis_value_hist": {str(k): int(v) for k, v in sorted(camvis_counter.items())},
...
def stage_proj(
    ...
    cam_sseg_reader = (
        SsegAuxReader(discover_aux_path(clip_dir, "sseg")) if discover_aux_path(clip_dir, "sseg") else None
    )
    ...
    sweep_idxs = sorted(set(int(i) for i in np.linspace(0, len(pc_ts) - 1, num=n_sweeps).round()))
    ...
    for pc_idx in sweep_idxs:
        ...
        sweep_rep = {
            ...
            "label_hist_top": _hist_named(np.bincount(labels, minlength=256), classes, top=5),
            "cameras": {},
        }
        ...
        for cam in cameras:
            ...
            cam_rep = {
                ...
                "visible_label_hist_top": _hist_named(np.bincount(vis_labels, minlength=256), classes, top=5),
            ...
            if cam_sseg_reader is not None and cam in sseg_cameras:
                ...
                try:
                    ...
                    try:  # some aux versions key by START timestamp
                        sseg_img = cam_sseg_reader.read(
                            cam, int(camera_sensor.frames_timestamps_us[frame_idx, ncore.data.FrameTimepoint.START])
                        )
                    except KeyError:
                        sseg_img = None
        ...
        sweep_rep["P(ignore|geom_visible)"] = round(float(ign[union_visible].mean()) if union_visible.any() else 0.0, 4)
        ...
        if camvis is not None:
            ...
            sweep_rep["geom_visible_but_camvis0_pct"] = round(100.0 * float((union_visible & (camvis == 0)).mean()), 2)
