...
**vast.ai 4090 视觉验收 ✅** (Norway 实例 37299842, $0.828/hr × ~1h 总成本 ~$0.83):
- **A800 不可用**：用户 confirm A800 做不了高斯渲染（Ampere datacenter SKU RT cores 阉割 → OptiX dlopen segfault, T8.10 已记录）。验收必须 vast.ai 4090+ 或本机 RTX
- **2 组 ckpt 对照测试** (9 张状态截图，详见 `docs/T8.14_vast_artifacts/README.md`):
  - **Run A** (`ckpt_with_viz_4d_v2.pt` schema_v1 pinhole, 5 张): baseline 50.8 FPS / sky off 49.9 / dyn off 56.0 / 3 关只留 dyn 63.3 / **全关 66.9 FPS (+31.7%)**
  - **Run B** (`ckpt_with_ftheta_v2.pt` schema_v2 FTheta, 4 张): baseline 6.91 FPS 清晰城市街景 + fisheye 桶形畸变 + cuboid wireframe / sky off 7.66 (bg 已学天空α≈1, sky 贡献小) / **bg+road+dyn 全 off 留 sky 7.66 sky-only 模糊环境编码** / **仅 dyn 留, 其他全关 8.16 FPS 99%纯黑 + 左上 dyn fisheye blob**
- **设计预期 100% 命中**: `_blend_sky` 短路 ✅ / `fused_view` 跳过禁用层 ✅ / `_empty_render` 兜底 ✅ / scene primitives 独立 ✅ / 零开销（性能随关闭层数线性提升）✅ / closure 默认参数绑定无错位 ✅
- **已知 follow-up** (非 T8.14 bug): GUI 状态 desync（浏览器刷新后 checkbox 与服务端 enabled_layer_names 不同步, 功能不受影响, 建议 `_on_client_connect` 拉一次 sync）；chrome MCP 截图无法落盘到 docs/ 因 base64 大数据被阻止 + osascript timeout → 证据以 chrome screenshot ID + README.md 描述形式归档（用户在对话中已直接看到 9 张状态截图）

**Plan**: `/Users/etendue/.claude/plans/threedgrut-playground-viser-gui-4d-py-g-pure-knuth.md` (10 task 全部 ✅, Mac + vast.ai 4090 双路验证完成)

**Commits**: 1c72fd8 (feat 代码+测试) / fb186c4 (docs plan+arch 同步) / 后续 commit 归档 vast.ai README + 实例销毁记录
...
**vast.ai California RTX 4090 实测** (2026-05-21, 实例 37206629, ~1h $0.98):
- ✅ install_env_uv.sh OptiX 编译 + viser==1.0.27 + 949MB ckpt rsync 全通
- ✅ **Variant A.2 pinhole 90°**: 矩形画面 + 灰色 motion blur 隧道 + cuboid 在中心 + 64 FPS. **与历史 `~/4dviz_artifacts/render_browser_fov_{45,75,140}deg.png` 同形态** → fov 假设证伪
- ✅ **Variant A.5 fisheye equirectangular 120°**: **椭圆 fisheye 框 + cuboid 在合理位置 + 天空/地面/建筑结构清晰可辨 + 58 FPS** → 结构性突破, 形态对了
- ⚠️ Variant A.5 内容仍模糊 → equirectangular fisheye ≠ NCore FTheta polynomial fisheye, 投影仍错配
- ➡️ **诊断锚定 Phase C (T8.13) 必走**: 3dgut UT rasterizer 通过 `Batch.intrinsics_FThetaCameraModelParameters` 才能正确投影 Gaussian 协方差, viz_4d schema 必须扩 FTheta 多项式系数

**最终的"为什么不是 fov 也不是 view_matrix"**:
- 用户之前 5 张 `render_first*.png` 截图实证: pinhole 任何 fov 都是同形态 motion blur, view_matrix 调整产生不同形态但仍乱糊
- 今天 vast.ai Variant A.5 实证: 切到本仓库已存在的 `_raygen_fisheye` 路径产生**形态完全不同**的椭圆 fisheye 输出 → 证明问题在投影模型, 不在 fov / 不在 c2w / 不在 LayeredGaussians blend
- **3dgut 是 rasterizer 不是 ray tracer**: 它用 intrinsics (而非 rays) 投影 Gaussian 中心 + 协方差到 2D 屏幕椭圆. FTheta-trained Gaussian 的 3D 形状已"反预补偿" FTheta 扭曲, 用 pinhole 或 equirectangular 投影都会产生双重扭曲 → 视觉乱糊

**Phase C 改动清单 (T8.13, ≥0.5d)**:
- `threedgrut/viz/metadata.py`: schema 加 `ego.primary_camera_intrinsics_FTheta` 字段 (结构抄 `datasetNcore.py:1467-1477`)
- `threedgrut_playground/utils/viz4d_metadata.py`: `FourDMetadata` 加 `ego_fisheye_params: Optional[dict]`
- `scripts/inject_viz_4d.py`: 老 ckpt 一键迁移补 FTheta 字段
- `threedgrut_playground/viser_gui_4d.py`: render path 检测到 FTheta 字段则构 `Batch.intrinsics_FThetaCameraModelParameters` 传 engine
- `threedgrut_playground/engine.py:_trace_scene_mog`: 加分支检测 FTheta intrinsics
- `tests/`: FTheta 路径数值正确性回归

**🚨 二次诊断更新 (基于 `~/4dviz_artifacts/render_*.png` 历史截图)**:
- `render_browser_fov_{45,75,140}deg.png` 三张截图**完全一样**形态（黑白隧道 motion-blur）→ **fov 不是问题**, 任何 fov 下都是同样形态的乱糊. T8.12 当时已手动试过 fov 试探, Phase A.2 (fov 90°) **几乎肯定不能修主问题**.
- `render_first_v3viewmatrix.png` / `render_first_v4camspace.png` / `render_first_realc2w.png` 三张截图**完全不同形态**（蓝色径向畸变 vs 黑白隧道）→ **view_matrix/c2w 约定调整 DID 改变输出**, 但仍非清晰街景, 像 "fisheye-trained Gaussians 被 pinhole 看到" 的径向模糊.
- **预期 vast.ai 验收结果**:
  - `--initial_fov_deg 90` (Phase A.2): 大概率与旧版几无差别（fov 不是真因）
  - `--camera_type Fisheye --camera_fov_deg 120` (Phase A.5, 利用本仓库已存在的 `_raygen_fisheye`): **最有希望**——切到 equirectangular fisheye raygen, 不是 FTheta polynomial 但比 pinhole 更接近 wide-fov 训练分布
  - 若 fisheye 路径也不彻底 → 进 Phase C (FTheta schema 扩展) 是必然路径
